...
def format_coordinates(lines, originalResolution, currentResolution):
    ...
    for val in lines.split(";"):
        ...
        for i in range(2):
            ...
            # print(p)
            p = (int((p[0]/resolution[0])*currentResolution[0]), int((p[1]/resolution[1])*currentResolution[1]))
...
for fileName in os.listdir(videoFolder):
    ...
    while bitrate > 50:
        ...
        decrease_bit_rate(filePath, newFilePath, bitrate)
    ...
    while resolutionWidth > 200:
        ...
        if not decrease_resolution(filePath, newFilePath, resolutionWidth):
